...
[EasyJobsBase](https://github.com/MineralsCloud/EasyJobsBase.jl).
...
2. Run the following commands and wait for them to finish:
   ...
   julia> Pkg.add("EasyJobsBase")
   ```

3. Run

   ```julia-repl
   julia> using EasyJobsBase
   ```
...
version of EasyJobsBase, type

```@repl
using Pkg
Pkg.update()
pkg"add https://github.com/MineralsCloud/EasyJobsBase.jl"
```
...
our [GitHub repository](https://github.com/MineralsCloud/EasyJobsBase.jl)
for new releases.
Once we release a new version, you can update EasyJobsBase by typing

```@repl
using Pkg
Pkg.update("EasyJobsBase")
Pkg.gc()
```

in the Julia REPL.

## Uninstall and reinstall EasyJobsBase
...
1. To uninstall, in a Julia session, run
   ...
   julia> Pkg.rm("EasyJobsBase")

   julia> Pkg.gc()
   ```

2. Press `ctrl+d` to quit the current session. Start a new Julia session and
   reinstall EasyJobsBase.
