...
[EasyJobs](https://github.com/MineralsCloud/EasyJobs.jl).
...
2. Run the following commands and wait for them to finish:
   ...
   julia> Pkg.add("EasyJobs")
   ```

3. Run

   ```julia-repl
   julia> using EasyJobs
   ```
...
version of EasyJobs, type

```@repl
using Pkg
Pkg.update()
pkg"add https://github.com/MineralsCloud/EasyJobs.jl"
```
...
our [GitHub repository](https://github.com/MineralsCloud/EasyJobs.jl)
for new releases.
Once we release a new version, you can update EasyJobs by typing
...
## Uninstall and reinstall EasyJobs
